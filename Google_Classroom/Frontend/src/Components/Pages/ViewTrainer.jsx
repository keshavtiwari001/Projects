...
const ViewTrainer = () => {
    ...
    async function fetch() {
        const response = await axios.get(`http://localhost:7001/trainer`, { headers: { "Authorization": `Barier ${token}` } })
    ...
    async function singleData(id) {
        console.log("id >>", id);
        const res = await axios.get(`http://localhost:7001/trainer/${id}`, { headers: { "Authorization": `Barier ${token}` } })
    ...
    return (
        ...
        <div className='mainDiv'>
            <h1>Trainer List</h1>
            <h1>{currentUser ? `hello, ${currentUser}` : ""}</h1>
