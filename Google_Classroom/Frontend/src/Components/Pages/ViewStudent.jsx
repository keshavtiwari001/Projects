...
const ViewStudent = () => {
    ...
    async function fetch() {
        const response = await axios.get(`http://localhost:7001/student`, { headers: { "Authorization": `Barier ${token}` } })
    ...
    async function singleData(id) {
        console.log("id >>", id);
        const res = await axios.get(`http://localhost:7001/student/${id}`, { headers: { "Authorization": `Barier ${token}` } })
    ...
    async function removeData(id) {
        console.log("id >>", id);
        const res = await axios.delete('http://localhost:7001/student/${id}', { headers: { "Authorization": `Barier ${token}` } })
        console.log('removed user >>', res.data)
    }
                ...
                {(
                    data && data.map((st, index) => (
                        <div key={index} style={{ border: "1px solid black", margin: "10px", padding: "10px" }}>
                            ...
                            <button className="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded" onClick={() => singleData(st._id)} >More Details</button>
                            <button className="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded" onClick={() => removeData(st._id)} >Remove Student</button>
                        </div>
                    ))
